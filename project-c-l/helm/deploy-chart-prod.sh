...
# Prérequis : Se connecter à Azure et au cluster de production
...
helm template ${SERVICE_NAME}-${NAMESPACE} \
  ...
  --set template.autoscaling.minBudgetReplicas=0 > /tmp/loop-charts/${SERVICE_NAME}-prod.manifest

# Appliquer le manifest
kubectl apply \
  -f /tmp/loop-charts/${SERVICE_NAME}-prod.manifest \
  --namespace ${NAMESPACE}
